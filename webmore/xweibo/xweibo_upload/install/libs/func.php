...
if(xwb_function_exists('date_default_timezone_set')) {
	date_default_timezone_set('Etc/GMT-8');
} else {
	putenv('Etc/GMT-8');
}

if (!xwb_function_exists('array_combine')) {
...
if (!xwb_function_exists('json_decode')){
...
if (!xwb_function_exists('json_encode')){
	include_once "servicesJSON.class.php";
...
function check_env(&$result)
{
	...
	/// 检查gd库版本
	if (xwb_function_exists('gd_info')) {
		$gd_info = gd_info();
	} else {
...
function check_app($app_key, $app_secret, &$http_res)
{
	...
	} else if (xwb_function_exists('curl', array('init', 'exec'))) {
...
function check_app_key($link, $db_name, $db_prefix = null)
{
	...
	if ($table_name) {
		...
		if (!empty($app_key) && !empty($app_secret) && WB_AKEY && WB_SKEY) {
			if (WB_AKEY != $app_key || WB_SKEY != $app_secret) {
				return '10001';
			}
		}
	}
