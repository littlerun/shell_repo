...
class xwbBBSplugin {
	...
	function updateBindUser($site_uid = false, $sina_uid = false, $access_token = false, $token_secret = false, $nickname = false)
	{
		...
		if ($nickname) {
			...
			$sql = 'SELECT * FROM ' . $this->db->getTable() . ' WHERE `sina_uid` = "' . $this->db->escape($sina_uid).'"';
	...
	function getBindUser($id = false, $type = 'site_uid')
	{
		...
		if ('sina_uid' == $type) {
			$where = ' `sina_uid` = "' . $this->db->escape($id) . '" AND uid != "" AND access_token != "" AND token_secret != ""';
		} else {
	...
	function getBatchBindUser($uids = false) 
	{
		...
		$sql = 'SELECT * FROM ' . $this->db->getTable() . ' WHERE `uid` in (' . $this->db->SQLIN(explode(',',$uids)) . ')';
