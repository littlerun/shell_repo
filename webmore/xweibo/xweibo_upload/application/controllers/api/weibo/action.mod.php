...
class action_mod
{
	 ...
	 * @var array
	 */
	var $_jsapi_ignore_sec_check = array('emotions', 'getprovinces');
	function action_mod()
	{
		Xpipe::usePipe(false);
		/// 判断是否是程序的api调用
		if (IS_IN_API_REQUEST) {
			...
			/// 检查用户身份是否已经绑定
			$db = APP :: ADP('db');
			$result = $db->getRow('SELECT * FROM '.$db->getTable(T_USERS).' WHERE uid = "'.$db->escape($api_uid).'"');
			if (empty($result) || 
			...
			$m = APP::getRequestRoute(true);
			if(!in_array(strtolower($m['function']), $this->_jsapi_ignore_sec_check)){
				if(!isset($_SERVER['HTTP_REFERER'])){
					APP::ajaxRst(null, 1040017, 'XWEIBO JS API SECURITY CHECK FAILURE');exit();
				}
    ...
    function getAtUsers()
    {
		...
		*/
		function sendNotice() {
			...
			//经检查，本逻辑主要用于对自己的定制提醒，故强制设置为对自身用户的操作
			$sina_uid = USER::uid();
			if($sina_uid < 1){
				APP::ajaxRst(false, 1010000, 'Parameter can not be empty');
			}
