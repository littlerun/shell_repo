...
class microLive 
{
	...
	function deleteLiveWb($id, $wb_id) {
		$sql = 'DELETE FROM ' . $this->db->getTable(T_MICRO_LIVE_WB) . ' WHERE live_id = "' . $this->db->escape($id) . '" AND wb_id = "' . $this->db->escape($wb_id) .'"';
	...
	function approveWb($id, $wb_id, $state = 1) {
		$sql = 'UPDATE ' . $this->db->getTable(T_MICRO_LIVE_WB) . ' SET state = "' . $this->db->escape($state) . '" WHERE live_id = "' . $this->db->escape($id) . '" AND wb_id = "' . $this->db->escape($wb_id) .'"';
		$ret = $this->db->execute($sql);
		if ($ret) {
			$this->_cleanCache(); //保存成功后清除缓存
		}
	...
	function getMicroLiveWbs($id, $state = null, $page = 1, $limit = 20, $last_wb_id = false, $params=array()) {
		$where = ' WHERE live_id = \'' . $this->db->escape($id). '\'';
		if ($state) {
			$where .= ' AND state = \'' . $this->db->escape($state). '\'';
		}
		if ($last_wb_id) {
			$where .= ' AND wb_id > "' . $this->db->escape($last_wb_id) . '"';
		}
		...
		$offset = ($page - 1) * $limit;
		$sql = 'SELECT wb_id, weibo, type, state FROM ' . $this->db->getTable(T_MICRO_LIVE_WB) . $where . ' ORDER BY add_time DESC LIMIT ' . (int)$offset . ', ' . (int)$limit;
	...
	{
		$where 	= ' WHERE live_id = "' . $this->db->escape($id) .'" AND state = "' . $this->db->escape($state). '"';	
		if ( isset($params['gMtype']) ) {
			$where .= " And type In (2, 3) ";
		}
