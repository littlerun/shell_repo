...
class InterviewWbAtme
{
	...
	function getAnswerList($interviewId, $askIdList)
	{
		...
		// Escape Var
		$askIdStr	 = $this->db->SQLIN($askIdList);
	...
	function getUserAskList($interviewId, $uid, $offset=0, $limit=40, $params=array())
	{
		...
		if ( $interviewId && $uid )
		{
			...
			if(isset($params['since_id'])){
				$params['since_id'] = $this->db->escape($params['since_id']);
				$sinceId =  " And `ask_id`>'{$params['since_id']}' ";
			}else{
				$sinceId =  ' ';
			}			
			...
			$this->db->checksql_set(array('sub_select_allow'=>true, 'union_select_allow'=>true));
	...
	function getUserAskCount( $interviewId, $uid, $params=array())
	{
		...
		if ( $interviewId && $uid )
		{
			$sinceId 	= isset($params['since_id']) ? (" And `ask_id` > '". $this->db->escape($params['since_id']). "' ") : '';
