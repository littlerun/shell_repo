...
class notice
{
	...
	*/
	function getNoticeNum($sina_uid = 0)
	{
		...
		$this->db->checksql_set(array('sub_select_allow'=>true));
	...
	*/
	...
	function _getUserNoticeStart($sina_uid)
	{
		...
		if ($sina_uid)
		{
			...
			{
				$rst['rst']['first_login'] = $this->db->escape($rst['rst']['first_login']);
	...
	*/
	...
	{
		$maxNoticeTime = $this->db->escape($maxNoticeTime);
		$sql = "SELECT COUNT(DISTINCT(`notice_id`)) FROM `" . $this->tb_recipients . "` WHERE (`recipient_id` = '" . $this->db->escape($sina_uid) . "' OR `recipient_id` = 0) AND `notice_id` IN (SELECT `notice_id` FROM " . $this->tb_notice . " WHERE `available_time` <= " . APP_LOCAL_TIMESTAMP . " AND `available_time` > '" . $this->db->escape($maxNoticeTime) . "')";
		$this->db->checksql_set(array('sub_select_allow'=>true));
		return RST($this->db->getOne($sql));
	}
