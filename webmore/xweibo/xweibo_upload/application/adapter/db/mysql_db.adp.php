...
class mysql_db extends interface_db 
{
	...
	function get($id, $table='', $id_name = 'id') {
		...
		$sql = 'SELECT * FROM ' . $table . ' WHERE `' . $this->escape($id_name) . '` =\'' . $this->escape($id) . '\'';
	...
	function delete($id, $table = '', $id_name = 'id') {
		...
		$sql = 'DELETE FROM ' . $table . ' WHERE `' . $this->escape($id_name) . '` IN(' . $ids_string. ')';
		if (!$this->execute($sql)) {
			return false;
		}
	...
	function save($data, $id = '', $table = '', $id_name = 'id') {
		...
		$sql = 'UPDATE ' . $table . ' SET ' . implode(',', $values) . ' WHERE `' . $this->escape($id_name) . '` =\'' . $this->escape($id) . '\'';
	...
	{	
		if(!defined('SQL_CHECK_SAFE') || SQL_CHECK_SAFE != false){
			...
			if(!$_checkIns->check($sql)){
				...
				$this->log('[execute][query_safe_error|'. $_err . ']|SQL:[' . $sql .']', array('REMOTE_ADDR'=>$remote_addr, 'ip'=>$detectip, 'url'=>$request_url));
