...
then
keyString='^.*$' #Set default regular expression.
fi
...
awk '$9 == 200 {print $7}' $i|egrep -i $keyString|sort|uniq -c|sed 's/^ *//g' | sed 's/\?.*$//g' | sed 's/id\/[0-9]*$//g' > $i.result	#Using the commands below to process each line of log files,print all succeeded request, and then counts the number of each request.
